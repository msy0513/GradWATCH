...
def edge_index_difference(edge_all, edge_except, num_nodes):
    ...
    mask = torch.from_numpy(np.isin(idx_all.to('cpu'), idx_except)).to(torch.bool)
...
def gen_negative_edges(edge_index, num_neg_per_node, num_nodes):
    ...
    j = torch.Tensor(np.random.choice(num_nodes, len(i), replace=True)).long().cuda()
...
def fast_batch_mrr_and_recall(edge_label_index, edge_label, pred_score, num_neg_per_node, num_nodes):
    ...
    # print(edge_neg.shape, score_idx.shape)
...
@torch.no_grad()
def report_rank_based_eval_meta(model, graph, x, edge_feature, label, label_edge_index, fast_weights, num_neg_per_node: int = 100):
    if num_neg_per_node == -1:
        # Do not report rank-based metrics, used in debug mode.
        return 0, 0, 0, 0
    
    # Get positive edge indices.
    # num_neg_per_node = label.shape[0] * 10
    edge_index = label_edge_index[:, label == 1]
    ...
    # print(new_edge_label.shape, new_edge_label_index.shape)

    # Construct evaluation samples.
    new_edge_label_index = new_edge_label_index.to('cpu').long()
    new_edge_label = new_edge_label.to('cpu').long()

    pred = model(graph, x, edge_feature, new_edge_label_index, fast_weights)
    pred = pred.to('cpu')

    mrr, recall_at = fast_batch_mrr_and_recall(new_edge_label_index, new_edge_label, pred, num_neg_per_node, graph.num_nodes())
